 ...
 *
 * For S3-backed images, transforms draft URLs to article URLs.
 * Falls back to CDN transformation for legacy Walrus-based media.
 *
 * @param content - The markdown content
 * @param articleId - The article ID for S3-backed images
 * @param quiltBlobId - Optional quilt blob ID for legacy Walrus CDN transformation
...
export function transformArticleMediaUrls(
  ...
  // First, transform any draft URLs to article URLs (S3-backed images)
  let transformed = transformDraftUrlsToArticleUrls(content, articleId);

  // Then, apply legacy Walrus CDN transformation for any remaining URLs
  if (quiltBlobId) {
