...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      ...
      // Redirect to sign-in ONLY from auth-required pages. Everything else is
      // public by default — an expired session or a 401 from an auth-only
      // widget must never bounce a visitor off a public page.
...
export interface MobileSessionStatus {
  status: 'pending' | 'scanned' | 'authenticated' | 'claimed' | 'expired';
  accessToken?: string;
  account?: {
    id: string;
    publicKey: string;
...
export const articlesAPI = {
  ...
  getRawContent: (quiltBlobId: string) =>
    api.get<ArrayBuffer>(`/articles/raw/${quiltBlobId}`, {
